...
/**
 * Performance test for the SalesChannelMaster entity.
 */
class SalesChannelMasterGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the SalesChannelMaster entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all salesChannelMasters")
            .get("/api/sales-channel-masters")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new salesChannelMaster")
            .post("/api/sales-channel-masters")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "salesChannel":"SAMPLE_TEXT"
                , "salesChannelDesc":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_salesChannelMaster_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created salesChannelMaster")
                .get("${new_salesChannelMaster_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created salesChannelMaster")
            .delete("${new_salesChannelMaster_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
