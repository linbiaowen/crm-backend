...
/**
 * Performance test for the ProductMms entity.
 */
class ProductMmsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductMms entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productMms")
            .get("/api/product-mms")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productMms")
            .post("/api/product-mms")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "mmsId":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productMms_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productMms")
                .get("${new_productMms_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productMms")
            .delete("${new_productMms_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
