...
/**
 * Performance test for the OfferSalesChannel entity.
 */
class OfferSalesChannelGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the OfferSalesChannel entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all offerSalesChannels")
            .get("/api/offer-sales-channels")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new offerSalesChannel")
            .post("/api/offer-sales-channels")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_offerSalesChannel_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created offerSalesChannel")
                .get("${new_offerSalesChannel_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created offerSalesChannel")
            .delete("${new_offerSalesChannel_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
