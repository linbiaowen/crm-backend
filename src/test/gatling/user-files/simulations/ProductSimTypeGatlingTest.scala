...
/**
 * Performance test for the ProductSimType entity.
 */
class ProductSimTypeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ProductSimType entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all productSimTypes")
            .get("/api/product-sim-types")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new productSimType")
            .post("/api/product-sim-types")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_productSimType_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created productSimType")
                .get("${new_productSimType_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created productSimType")
            .delete("${new_productSimType_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
