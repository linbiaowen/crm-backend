...
public class DataServiceSpecResource {
    ...
    public ResponseEntity<List<DataServiceSpec>> getAllDataServiceSpecs(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("cfsservice-is-null".equals(filter)) {
            log.debug("REST request to get all DataServiceSpecs where cfsService is null");
            return new ResponseEntity<>(dataServiceSpecService.findAllWhereCfsServiceIsNull(),
                    HttpStatus.OK);
        }
